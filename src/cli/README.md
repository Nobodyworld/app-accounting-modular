...
- `provider_sdk.py` – Provider SDK command group for deterministic configured/module conformance evidence and path-safe provider scaffolding. It is mounted as `provider-sdk` under `cli.macli` and may also be invoked directly during isolated SDK development.
