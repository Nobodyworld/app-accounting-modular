...
def validate_file_links(file_path: Path, root: Path) -> list[LinkIssue]:
    ...
    try:
        ...
        return [
            LinkIssue(
                file=str(file_path.relative_to(root)), line_num=None, link="", issue_type="read_error", details=str(e)
            )
        ]
    ...
    for file_path in files:
        ...
        if file_path.suffix in (".md",):
            try:
                content = file_path.read_text(encoding="utf-8")
                links = extract_links_from_markdown(content, file_path)
                links_checked += len([link for link in links if not is_external_link(link[2])])
                assets_checked += len([link for link in links if link[1] in ("image", "html_img")])
            except OSError:
                pass
