...
def test_post_transaction():
    with Session(engine) as s:
        ls = LedgerService(s)
        acct = ls.create_account("Cash", "ASSET", "1000")
        txn = ls.post_transaction(date.today(), "Test", [{"account_id": acct.id, "debit": 100, "credit": 0, "currency": "USD"}])
...
def test_trial_balance():
    with Session(engine) as s:
        ls = LedgerService(s)
        acct = ls.create_account("Cash", "ASSET", "1000")
        ls.post_transaction(date.today(), "Test", [{"account_id": acct.id, "debit": 100, "credit": 0, "currency": "USD"}])
