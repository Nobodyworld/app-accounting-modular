...
- **Fallback**: On provider failure, rerun with `--fx-provider`, `--commodity-provider`, or `--tax-provider` overrides for targeted retries. See `src/cli/macli.py` and `src/apps/api/routers/snapshot.py`.
...
- **Fallback**: On repeated failures, disable the provider in configuration and notify the Snapshot Steward Agent. See `src/cli/macli.py`.
...
- **Checklist**:
  ...
  2. Use `make audit` or `python -m tools.audit_metrics --skip-trace --format markdown` to compute aggregate coverage percentages.
  3. Attach the resulting metrics to `docs/reports/` or release notes as needed.
- **Fallback**: If trace output is noisy, prune standard-library `.cover` files before archiving to focus on project modules.
