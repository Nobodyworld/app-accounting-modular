...
v0.5 closes the external-author gap. The authoritative implementation lives in `packages/provider-sdk/src/modular_accounting_provider_sdk`; `apps.provider_sdk` re-exports the same public objects. The SDK declares that implementation as a zero-dependency in-tree PEP 517 backend. Generated providers declare the exact installed SDK distribution as their backend requirement. The repository-owned acceptance harness uses the standard build frontend, rebuilds extracted source-layout sdists, installs SDK and provider wheel/sdist variants in separate disposable environments with no repository `PYTHONPATH`, validates metadata and RECORD entries, blocks network connections during structural validation, proves application packages are absent, and executes pre-allowlist rejection through v0.4 reconciliation, organization policy/default resolution, and process-trust removal.

Package and archive inputs are fail-closed. Module strings must be bounded dotted identifiers ending in `.provider`; source and force-regeneration targets must remain beneath ordinary resolved roots without links or reparse points; and artifact evidence rejects absolute, drive, UNC, traversal, duplicate, link, special-file, and oversized members. Stable CLI categories replace raw filesystem exception text.
