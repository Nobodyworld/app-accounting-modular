...
7. Review the already approved posted-journal request and load its separately paged append-only decision history. Approval summaries and history are each bounded; a requestor cannot approve their own request.
...
These tests prove the legal/illegal lifecycle, two-person controls, inclusive direct/workflow READY and CLOSED posting locks, deterministic close-first and posting-first outcomes for both posting paths, ledger-revision staleness, no partial writes, single-snapshot conditional evidence persistence, recorded-download enforcement, manifest equality, final close, and explicit reopen.
