...
| Work-slice storage hygiene | Documentation proposal open | PR #147 proposes repository-level worktree and temporary-workspace cleanup rules that require ignored files, databases, user data, and other artifacts to be classified before deletion. | Reconcile it separately; Git ignore status alone must never be treated as proof that local data is disposable. |
...
| Container onboarding | Validated for local demonstration | Digest-pinned API and Streamlit images, hash-locked dependencies, Compose configuration, least-privilege checks, SBOMs, checksums, and trusted-event attestations are present. PR #139 merged the coordinated Python-base, runtime-provider, and tooling maintenance contract. | Preserve equivalent container validation while avoiding redundant workflows when enforcement can be consolidated safely. |
...
4. Align documented acceptance gates and repository merge enforcement; do not remove an existing required gate until equivalent or stronger enforcement is in place.
5. Reconcile PR #147 separately and preserve its stricter rule that ignored databases, environment files, user data, and unknown artifacts require explicit classification before cleanup.
