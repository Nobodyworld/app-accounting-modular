...
| Adapter SDK foundation | Partial | Typed provider contracts, loader services, catalog metadata, caching behavior, bounded outbound-provider behavior, and reference adapters exist. | Package a documented third-party adapter SDK with conformance tests after the v0.2 maintenance closeout. |
...
| Ruff tooling policy | Complete | Ruff 0.16.0 is deliberately pinned with explicit lint selection, Python-only discovery, Markdown exclusion, and executable regression coverage. | Evaluate patch updates only through the same explicit policy and exact-head validation boundary. |
...
| Container onboarding | Validated for local demonstration | Digest-pinned API and Streamlit images, hash-locked dependencies, Compose configuration, least-privilege checks, SBOMs, checksums, and trusted-event attestations are present. | Complete issue #138 as the coordinated Python-base, runtime-provider, and maintenance closeout before broader dependency work. |
...
1. Complete issue #138 as one coordinated Python base-image, yfinance compatibility, Ruff patch, generated-lock, and repository-state closeout.
