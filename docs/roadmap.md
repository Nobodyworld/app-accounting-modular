...
| Provider Author Kit | Active v0.5 tranche in issue #148 / draft PR #149 | The current scaffold and conformance engine work inside the application source tree, but a clean external author cannot yet install a standalone SDK artifact, generate a conventional provider project, build/install it in isolated environments, and produce operator handoff evidence without application imports. | First remove audited stale local worktrees and branches, then deliver the installable SDK package, standalone author CLI and scaffold, clean-environment acceptance, compatibility/deprecation lifecycle, and allowlist-first v0.4 handoff proof as one coherent slice. |
...
| Ruff tooling policy | Complete on main; stale patch proposal closed | `main` deliberately pins Ruff 0.16.2 with explicit lint selection, Python-only discovery, Markdown exclusion, and executable regression coverage. PR #142 was closed during pre-v0.5 pruning because no vulnerability or current compatibility need justified carrying the stale branch. | Start any future Ruff update from then-current `main` as a fresh maintenance slice only when security, compatibility, or policy evidence justifies it. |
| Development-tool dependency maintenance | Stale proposals closed | PRs #143 and #144 were closed without merge because the pytest-cov 7.x and mypy 2.x major-range proposals were not vulnerability-driven and should not overlap v0.5. | Evaluate future major updates separately from then-current `main` with explicit compatibility and policy evidence. |
...
1. Remove the audited stale local worktrees and their local branches before allocating a new v0.5 worktree; preserve and report any dirty or unexpected state rather than deleting it.
2. Deliver issue #148 / draft PR #149 as one v0.5 Provider Author Kit tranche: standalone SDK distribution, standalone scaffold/CLI, clean-environment wheel/sdist proof, compatibility lifecycle, deterministic evidence, and explicit v0.4 operator handoff.
3. Preserve `settings.allowed_providers` as the only executable provider trust source; packaging, importability, manifests, entry points, and persistence must never self-authorize.
4. Reconcile stale v0.4 candidate/draft wording across documentation while implementing v0.5; do not create a separate cosmetic release-state PR.
5. Keep documentation PR #147 isolated from the product branch. PRs #142, #143, and #144 are closed and must not be revived or copied into v0.5.
6. Keep setup and authoring instructions executable from clean Windows, macOS, Linux, and container-compatible environments where applicable.
7. Convert the broader application into a conventionally installable Python project only after the standalone SDK package boundary is proven; do not force all application packaging into v0.5.
8. Expand strict typing beyond the current targeted modules only when it directly supports the package, compatibility, or trust boundary under review.
9. Extend the validated v0.2 close controls only through explicit policy decisions and measured critical-module coverage.
