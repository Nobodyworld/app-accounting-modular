...
| Provider Author Kit | Implemented for v0.5 Early Beta | Draft PR #149 contains the authoritative in-tree PEP 517 backend, compatibility facade, standalone CLI, typed provider scaffold, wheel/sdist and extracted-source builds, clean installs, metadata/RECORD and path-safety evidence, and the executed allowlist/v0.4/trust-removal handoff. | Complete exact-head local and hosted validation; keep publication, registry, marketplace, and certification out of this tranche. |
...
1. Complete exact-head quality, changed-production coverage, accounting-control, dependency, secret, package, and hosted container evidence for draft PR #149.
2. Preserve the implemented v0.5 standalone SDK/facade boundary, authoritative declared backend, offline wheel/sdist acceptance, and executed governance handoff.
...
4. Keep v0.4 governance terminology aligned with the merged trust boundary; do not create a separate cosmetic release-state PR.
