...
| Accountant close workspace | Complete for v0.2 Early Beta | Tenant-scoped periods, posting locks, reconciliations, budget variances, staged blockers, journal approvals, checklist readiness, deterministic evidence, API routes, and Streamlit workflow are present. | Collect hosted exact-head and operator acceptance evidence; do not expand the claim to ERP or production close certification. |
